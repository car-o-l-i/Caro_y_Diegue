...
int santa_melody[] = {
  ...
  NOTE_A4, 8,NOTE_G4,8, NOTE_F4,4, NOTE_F4,4,
...
void songSC(int buzzer) {
  int tempo = 120;
  int notes = sizeof(santa_melody) / sizeof(santa_melody[0]) / 2;
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
    ...
    divider = santa_melody[thisNote + 1];
    if (divider > 0) {
    ...
    tone(buzzer, santa_melody[thisNote], noteDuration*0.9);
...
void jingleBells(int melodyPin){
  ...
  for (int thisNote = 0; thisNote < size; thisNote++) {
      ...
      int noteDuration = 1000 / tempoJB[thisNote];
